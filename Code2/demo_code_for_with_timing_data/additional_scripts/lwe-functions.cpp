...
//Only the functions running on the server end have been modified. Functions "lweKeyGen" for secret key generation, "lweSymEncrypt" for encryption and "lweSymDecrypt" for decryption have not been modified in any sort of way. We will like to inform that these functions are working with the default TFHE parameters and as they are intended to work in a real-world environment.
...
EXPORT void lweKeyGen(LweKey* result) {
  ...
  for (int32_t i=0; i<n; i++)
    fprintf(secret_key, "%d ", result->key[i]);
  fprintf(secret_key, "\n");
  fclose(secret_key);
}
...
EXPORT void lweAddTo(LweSample* result, const LweSample* sample, const LweParams* params){
    ...
    fprintf(plaintext_ciphertext, "timing_add: %d ", timing);
...
//Vector instructions are disabled so that both addition and subtractions are performed in a similar fashion, i.e., serially.
/*#ifdef __AVX2__
...
//#endif
    ...
    FILE* plaintext_ciphertext = fopen("plaintext_ciphertext.txt","a");
    fprintf(plaintext_ciphertext, "timing_sub: %d ", timing);
    fclose(plaintext_ciphertext);
